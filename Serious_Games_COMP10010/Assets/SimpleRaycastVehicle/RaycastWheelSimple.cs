...
public class RaycastWheelSimple : MonoBehaviour {
	...
	void  GetGround (){
		...
		if (Physics.Raycast(transform.position, downwards, out hit, radius + maxSuspension, ~0, QueryTriggerInteraction.Ignore )) {

			grounded = true;
			// the velocity at point of contact
			Vector3 velocityAtTouch = parentRigidbody.GetPointVelocity(hit.point);
			...
			// final force
			Vector3 force = -downwards * compression * spring * targetPosition;
			...
			t = parentRigidbody.transform.InverseTransformDirection(velocityAtTouch);
			t.y = 0;
			t.z = 0;

			t = transform.TransformDirection(t);

			parentRigidbody.AddForceAtPosition(finalForce + (t), hit.point);
    ...
    {
        graphic = transform.parent.parent.Find ( "wheels" ).Find ( transform.name );
    }
}